...
public sealed partial class Z80GB
{
    ...
    private void SUB_N() => Sub(Fetch(), 8);
    ...
    private void ADD_A_N() => ADD(Fetch(), 8);
    private void RET_C()
    {
        _clock.cycles += 4;
        if (_registers.FlagC)
    ...
    private void JR_NZ_N()
    {
        ConditionalJump(_registers.FlagZ == false);
    }
    ...
    private void ADD(Bit8Value b1, byte cycles = 4)
    {
        Bit8Value current = _registers.A;
        var result = current.Value + b1.Value;

        _registers.FlagN = false;
        _registers.FlagZ = result == 0;
        _registers.FlagC = result > byte.MaxValue;
        _registers.FlagH = current.Value <= 0xf && (_registers.FlagC || result > 0xf);

        _registers.A = (byte)result;
        _clock.cycles += cycles;
    }
    private void Sub(Bit8Value value, byte cycles = 4)
    {
        Bit8Value current = _registers.A;
